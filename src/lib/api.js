 ...
 * API Functions
 */
import { ErrorMessages, APIConfig } from '../config/index';
...
function fetcher(method, inputEndpoint, inputParams, body) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if(inputEndpoint !== 'register/' && inputEndpoint !== 'rest-auth/login/'){
    ...
    return fetch(thisUrl, req)
      .then(async (rawRes) => {
        ...
        try {
          ...
          throw { message: ErrorMessages.invalidJson };
        }
      ...
      .then((res) => {
        if (res && res.status === 403) { return res; }
 ...
 * - So we can call AppAPI.chapter.get() for example
 */
const AppAPI = {
  handleError,
};
